...
class Filter {
    ...
    constructor(filters : NodeListOf<HTMLButtonElement>, allowMultipleSelections : Boolean = false) {
    ...
    #filterItems(ev : MouseEvent) {
        const filterBtn = ev.currentTarget;

        if(filterBtn === null) return;
        if(!(filterBtn instanceof HTMLButtonElement)) throw new Error('Function filterItems can only be used when clicking on HTMLButtonElement instances.');

        const isExpanded = (filterBtn.getAttribute('aria-expanded') === 'true');

        if(this.#allowMultipleSelections) {
            if(this.#activeFilters === 1 && isExpanded) return;
    ...
    #getControlledItem(filterBtn : HTMLButtonElement) : HTMLElement {
        const controlledItemId = filterBtn.getAttribute('aria-controls');
        if(controlledItemId === null) throw new Error('Element ' + filterBtn + ' need to have an attribute aria-controls.');

        const controlledItem = document.getElementById(controlledItemId);
        if(controlledItem === null) throw new Error('Attribute aria-controls on element ' + + ' doesn\'t correspond to any element in the DOM');
