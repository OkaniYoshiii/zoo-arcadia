...
class Dialog {
    ...
    constructor(openBtn : Element) {
        if(!(openBtn instanceof HTMLButtonElement)) throw new Error('Argument 1 (openBtn) must be an instanceof HTMLButtonElement. Received : ' + Object.getPrototypeOf(openBtn))
        this.#openBtn = openBtn;
        const dialogId = this.#openBtn.getAttribute('data-dialog-id');
        if(!dialogId) throw new Error('openBtn must have a data-dialog-id attribute. This attribute should reference the id of the dialog you want to open.');
        const dialog = document.getElementById(dialogId);
        if(!dialog) throw new Error(`No element found with id ${dialogId}. Change data-dialog-id on your button to match an existing element.`)
        if(!(dialog instanceof HTMLDialogElement)) throw new Error(`Element with id of ${dialogId} is not an instance of HTMLDialogElement.`);
    ...
    init() {
        ...
        if(!dialogContent) throw new Error('Dialog must have a dialog__content element inside of it.');
