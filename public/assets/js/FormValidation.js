...
class FormValidation {
    ...
    #validateForm(ev) {
        ...
        for (const [key, value] of formData) {
            ...
            if (key === 'email') {
                if (value === '')
                    this.#errors.push('L\'email ne doit pas être vide.');
                if (!emailRegexp.test(value.toString()))
                    this.#errors.push('L\'email doit commencer par une suite de caractères, puis un arobase, une autre suite de caractères et une extension (.com par exemple).');
            }
        }
        this.#showErrors();
    }
